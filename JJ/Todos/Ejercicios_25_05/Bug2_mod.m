%Bug2_mod Bug navigation class
%
% A concrete subclass of the abstract Navigation class that implements the Bug2_mod 
...
%         bug = Bug2_mod(map);      % create navigation object
%         start = [20,10]; 
%         goal = [50,35];
...
classdef Bug2_mod < Navigation
    ...
    methods
        ...
        function bug = Bug2_mod(varargin)
            %Bug2_mod.Bug2_mod Construct a Bug2_mod navigation object 
            %
            % B = Bug2_mod(MAP, OPTIONS) is a Bug2_mod navigation object, and MAP is an occupancy grid,
        ...
        function pp = query(bug, start, goal, varargin)
            ...
            % - The Bug2_mod algorithm is completely reactive so there is no planning
            %   method.
        ...
        function n = next(bug, robot)
            ...
            % implement the main state machine for Bug2_mod
            n = [];
            robot = robot(:);
            ...
            if bug.step == 1
                ...
                if bug.isoccupied(robot + [dx; dy])
                    ...
                    bug.k = numcols(bug.edge);  % skip the first edge point, we are already there
                else
                    n = robot + [dx; dy];
                end
            end % step 1
            ...
            if bug.step == 2
                ...
                if bug.k > 1
                    ...
                    error('RTB:Bug2_mod:noplan', 'robot is trapped')
                    return;
                end
        ...
        function plan(bug)
            error('RTB:Bug2_mod:badcall', 'This class has no plan method');
        end
